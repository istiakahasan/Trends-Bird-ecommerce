    ...
    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm space-y-4">
      <div className="flex justify-between items-center">
        ...
        <PermissionGuard permission="brand:create">
          <Link to="/brands/create" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors">
            + New Brand
          </Link>
        </PermissionGuard>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="text-sm text-gray-600 divide-y divide-gray-50">
            {brands.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-4 text-center text-gray-400">No brands found.</td>
              </tr>
            ) : (
              brands.map((b: any) => (
                <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                  <td className="py-3 pl-2 font-semibold text-gray-800">{b.name}</td>
                  <td className="py-3 font-mono text-xs bg-gray-50 px-2 py-0.5 rounded text-gray-600 inline-block mt-2">{b.slug}</td>
                  <td className="py-3">{b.description}</td>
                  <td className="py-3 text-right pr-2 space-x-2">
                    <PermissionGuard permission="brand:update">
                      <Link to={`/brands/${b.id}`} className="text-blue-600 hover:text-blue-800 font-semibold">Edit</Link>
                    </PermissionGuard>
                    <PermissionGuard permission="brand:delete">
                      <button onClick={() => handleDelete(b.id)} className="text-red-600 hover:text-red-800 font-semibold">Delete</button>
                    </PermissionGuard>
                  </td>
                </tr>
              ))
